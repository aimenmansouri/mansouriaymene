...
<div class=" bg-gray-100 max-w-lg m-auto p-5 rounded-lg shadow-lg">
    <h1 class="text-2xl mb-5">Edit student info :</h1>
    <form action="" method="POST">
        ...
        <hr class="my-3">
        <h2 class="text-lg mb-5">Student info :</h2>
        <!--Admin table fields-->
        <fieldset>
            <legend class="block text-sm font-medium text-gray-700">Matricule :</legend>
            <div class="rounded-md -space-y-px mb-5">
            <div>
                <label for="matricule" class="sr-only">Matricule</label>
                <input value="{{ $student_info->matricule }}" type="text" name="matricule" id="card-number" class="mb-3 hover:cursor-pointer focus:ring-sky-500 focus:border-sky-500 relative block w-full  rounded bg-transparent focus:z-10 sm:text-sm border-gray-300 " placeholder="Matrucule">
            </div>
            <div class="flex -space-x-px ">
                <div>
                    <label for="groupe" class="block text-sm font-medium text-gray-700">Groupe</label>
                    <select id="groupe" name="groupe_id" class="hover:cursor-pointer mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none  focus:border-sky-500 sm:text-sm rounded-md">
                        @foreach ($groups as $group)
                        <option
                        @if ($group->id == $student_info->groupe_id)
                        selected
                        @endif
                         value="{{ $group->id }}">year {{ $group->year }} group n {{ $group->groupe }} speciality {{ $group->speciality }}</option>
                        @endforeach
                    </select>
                    </div>
                </div>
