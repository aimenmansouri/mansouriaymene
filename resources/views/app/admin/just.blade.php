...
<div class="w-2/3 lg:w-1/3 m-auto">
    <form action="" method="GET">
        <label for="search" class="block text-sm font-medium text-gray-700">Search student / added by</label>
...
<div class="flex">
    <div class="">
        <h2 class="text-xl mb-5">Justification :</h2>
    </div>
</div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900">
                        {{$just->student_name}} {{$just->student_surname}}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">
                        {{$just->added_by_name}} {{$just->added_by_surname}}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">
                        {{$just->updated_at}}
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <form action="{{ route('just_details') }}" method="POST">
                            ...
                            <button href="#" type="submit" class="underline ml-1 text-green-800">
                            + Info
                            </button>
                        </form>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <form action="" method="POST" onsubmit="return confirm('Do you really want delete justification ?');">
                            @csrf
                        <button type="submit" class="text-red-600 hover:text-red-900">Delete</button>
                        <input type="hidden" name="student_id" value="{{ $just->student_id }}">
                        <input type="hidden" name="just_id" value="{{ $just->id }}">
                        </form>
                    </td>
                </tr>
...
<script>
    var groups = document.querySelector('#just');
    groups.classList.add("bg-gray-900");
    var groups = document.querySelector('#just-d');
    groups.classList.add("bg-gray-900");
</script>
@endsection
